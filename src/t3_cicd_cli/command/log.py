...
@click.option(
    "--job",
    type=str,
    help="The job name to query the logs for."
)
def log(pipeline: str, stage: str, job: str):
    ...
    if pipeline and stage and job:
        click.echo(f"Shows logs for job {job} in " +
                   f"pipeline {pipeline}, stage {stage}.")
    if pipeline and stage:
        click.echo("Shows all logs for the jobs in " +
                   f"pipeline {pipeline}, stage {stage}.")
    if pipeline:
        click.echo("Shows all logs for the jobs and stages in " +
                   f"pipeline {pipeline}.")
